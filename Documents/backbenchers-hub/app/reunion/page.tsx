...
const EVENT_CONFIG = {
  ...
  currentCategoryName: "Early Bird", // e.g., 'Early Bird', 'Pre-Reg', 'General'
  registrationFee: "512 BDT",
  isRegistrationOpen: true,
  tickifyLink: "https://tickify.live/events/backbenchers24", // Placeholder
...
export default function ReunionSquidEdition() {
  ...
  return (
    <main className="bg-[#050505] min-h-screen text-white selection:bg-[#ff005a]/30 font-satoshi overflow-x-hidden">
      
      {/* 1. HERO SECTION */}
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-20 px-6 overflow-hidden">
        {/* Abstract Pink Neon Background Glow */}
        ...
        >
          ...
          <motion.h1 variants={fadeInUp} className="font-monument text-3xl md:text-6xl uppercase tracking-tighter mb-6">
          ...
          <motion.div variants={fadeInUp} className="flex gap-4 md:gap-8 justify-center mb-16">
            {Object.entries(timeLeft).map(([unit, value], i) => (
              <div key={i} className="flex flex-col items-center">
                <div className="text-2xl md:text-5xl font-monument text-white border-b-2 border-[#ff005a]/50 pb-2">
                  {value < 10 ? `0${value}` : value}
                </div>
                <span className="text-[8px] md:text-[10px] uppercase tracking-widest mt-3 text-gray-500">{unit === 'd' ? 'Days' : unit === 'h' ? 'Hours' : unit === 'm' ? 'Mins' : 'Secs'}</span>
              </div>
            ))}
          </motion.div>
        </motion.div>
      ...
      <section className="py-24 px-6 relative">
        <div className="container max-w-4xl mx-auto">
          ...
          >
            {/* Background Symbols */}
            <div className="absolute top-5 right-10 text-white/5 font-monument text-7xl select-none">○ △ □</div>
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#ff005a]/10 border border-[#ff005a]/30 mb-6">
                  ...
                  <span className="text-[#ff005a] text-[10px] font-monument uppercase tracking-widest">{EVENT_CONFIG.currentCategoryName} LIVE</span>
                </div>
                <h2 className="text-4xl font-monument mb-4 uppercase leading-tight italic">Secure Your <br /> Player Entry</h2>
              ...
              <div className="bg-black/40 border border-white/5 rounded-3xl p-8 text-center flex flex-col items-center justify-center">
                ...
                <div className="text-5xl font-monument text-white mb-8">{EVENT_CONFIG.registrationFee}</div>
                
                <a 
                  href={EVENT_CONFIG.isRegistrationOpen ? EVENT_CONFIG.tickifyLink : "#"}
                  className={`w-full py-5 rounded-xl font-monument text-[10px] uppercase tracking-[0.3em] flex items-center justify-center transition-all ${
      ...
      <section className="py-24 px-6 border-t border-white/5">
        <div className="container max-w-4xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="font-monument text-2xl uppercase tracking-widest mb-4 italic">Access Tiers</h2>
            ...
            {EVENT_CONFIG.feeStructure.map((tier, idx) => (
              ...
              >
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h3 className="font-monument text-sm uppercase">{tier.label}</h3>
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest">{tier.status}</p>
                  </div>
                </div>
                <div className="font-monument text-lg text-white">{tier.fee}</div>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              >
                ...
                <h3 className="font-monument text-xs uppercase mb-4 tracking-widest">{item.title}</h3>
      ...
      <footer className="py-20 border-t border-white/5 text-center">
        <div className="font-monument text-[10px] text-gray-600 uppercase tracking-[1em] mb-4">
          BackBencher's '24 | Digital Frontier
        </div>
        <div className="flex justify-center gap-6 text-white/20 font-monument text-xl">
           ○ △ □
        </div>
      </footer>
